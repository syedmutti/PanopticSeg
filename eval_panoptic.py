...
def generate_instance_segmentation(predictions_semantic, predictions_center_points, predictions_offset_vectors):
    ...
    keep_mask = tf.greater(predictions_center_points, 12.75)
    ...
    def false_fn(x):
        return tf.zeros(tf.shape(squeezed_heatmap))
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    instance_prediction = generate_instance_segmentation(predictions_semantic, predictions_center_points, predictions_offset_vectors)
    ...
    category_label = tf.cast(category_label * tf.cast(not_ignore_mask, tf.int32), tf.int32)
    instance_label = tf.squeeze(samples[common.LABEL_INSTANCE_IDS][0])
    category_prediction = category_prediction * tf.cast(not_ignore_mask, tf.int64)
    instance_prediction = instance_prediction * tf.cast(not_ignore_mask, tf.int64)
